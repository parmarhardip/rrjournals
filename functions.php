...
require get_template_directory() . '/inc/meta-box-callbacks.php';
		...
		<tbody>
			<?php
				while ($issue_posts -> have_posts()) : $issue_posts -> the_post(); 
					...
					$icp_year_and_month = rr_get_field( 'icp_year_and_month', $issue_id ); 
					$icp_page_number = rr_get_field( 'icp_page_number', $issue_id ); 
					$icp_authors_names_loop = rr_get_field( 'icp_authors_names', $issue_id ); 					
					$ice_paper_category = rr_get_field( 'ice_paper_category', $issue_id ); 					
					$ice_subject = rr_get_field( 'ice_subject', $issue_id ); 					
					$ice_pdf_upload = rr_get_field( 'ice_pdf_upload', $issue_id );
...
function the_single_issue_content_html( $issue_id, $issue_title, $issue_count ) {
	...
	$icp_year_and_month        = rr_get_field( 'icp_year_and_month', $issue_id );
	$icp_doi                   = rr_get_field( 'icp_doi', $issue_id );
	$icp_page_number           = rr_get_field( 'icp_page_number', $issue_id );
	$icp_authors_names_loop    = rr_get_field( 'icp_authors_names', $issue_id );
	$ice_paper_category        = rr_get_field( 'ice_paper_category', $issue_id );
	$ice_subject               = rr_get_field( 'ice_subject', $issue_id );
	$ice_pdf_upload            = rr_get_field( 'ice_pdf_upload', $issue_id );
	$ice_google_drive_pdf_link = rr_get_field( 'ice_google_drive_pdf_link', $issue_id );
	$icp_abstract_content      = rr_get_field( 'icp_abstract_content', $issue_id );
	$postcat                   = get_article_category( $issue_id );
	?>
	<tr>
		<td colspan="3">
	...
	<tr>
		<td></td>
		<?php					
		$file_id = rr_get_field_raw( 'ice_pdf_upload', $issue_id );					
		...
		<td colspan="2">
			<ul class="indLnk">
				...
				$ice_mla 			= rr_get_field( 'ice_mla', $issue_id );
				$ice_apa 			= rr_get_field( 'ice_apa', $issue_id );
				$ice_chicago 		= rr_get_field( 'ice_chicago', $issue_id );
				$ice_harvard 		= rr_get_field( 'ice_harvard', $issue_id );
